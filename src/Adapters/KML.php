...
class KML extends GeoAdapter
{
    ...
    private function linestringToKML(Collection $geom, $type = false)
    {
        if (!$type) {
    ...
    public function polygonToKML(Collection $geom)
    {
        $components = $geom->getComponents();
        $str = '';

        if (!empty($components)) {
            $str = '<' . $this->nss . 'outerBoundaryIs>' . $this->linestringToKML($components[0],
                    'LinearRing') . '</' . $this->nss . 'outerBoundaryIs>';
    ...
    public function collectionToKML(Collection $geom)
    {
